...
- **Connection** — built from `PG_HOST`/`PG_PORT`/`PG_USER`/`PG_PASSWORD`/`PG_DATABASE` with `?sslmode=require` appended explicitly; the managed host refuses insecure connections
